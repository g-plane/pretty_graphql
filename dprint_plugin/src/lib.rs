...
pub struct PrettyGraphqlPluginHandler;

impl SyncPluginHandler<FormatOptions> for PrettyGraphqlPluginHandler {
    fn plugin_info(&mut self) -> SyncPluginInfo {
        ...
        SyncPluginInfo {
            ...
            file_matching: FileMatchingInfo {
                file_extensions: ["graphql", "gql"].into_iter().map(String::from).collect(),
                file_names: vec![],
            },
        }
    }
...
#[cfg(target_arch = "wasm32")]
generate_plugin_code!(PrettyGraphqlPluginHandler, PrettyGraphqlPluginHandler);
